...
pub fn download_model(_name: []const u8, _cache_dir: ?[]const u8) KreuzbergError![]u8 {
    const _name_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{_name}, 0);
    defer std.heap.c_allocator.free(_name_z);
    const _cache_dir_z: ?[:0]u8 = if (_cache_dir) |v| try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{v}, 0) else null;
    defer if (_cache_dir_z) |z| std.heap.c_allocator.free(z);
    const _result = c.kreuzberg_download_model(_name_z, if (_cache_dir_z) |z| z.ptr else null);
    if (c.kreuzberg_last_error_code() != 0) {
        return _error_with_message(KreuzbergError);
    }
    if (_result == null) {
    ...
    const _result_len = c.kreuzberg_download_model_len(_name_z, if (_cache_dir_z) |z| z.ptr else null);
    return blk: {
        if (_result == null) return _first_error(KreuzbergError);
        const slice = _result[0.._result_len];
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
pub fn get_embedding_preset(_name: []const u8) error{OutOfMemory}!?[]u8 {
    const _name_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{_name}, 0);
    defer std.heap.c_allocator.free(_name_z);
    const _result = c.kreuzberg_get_embedding_preset(_name_z);
    return if (_result == null) null else blk: {
        const _json_ptr = c.kreuzberg_embedding_preset_to_json(_result.?);
        c.kreuzberg_embedding_preset_free(_result.?);
        defer _free_string(_json_ptr);
...
///
/// Since v5.0.
pub fn rerank(_query: []const u8, _documents: []const u8, _config: []const u8) KreuzbergError![]u8 {
    ...
    const _result = c.kreuzberg_rerank(_query_z, _documents_z, _config_handle);
    if (c.kreuzberg_last_error_code() != 0) {
        return _error_with_message(KreuzbergError);
    }
    if (_result == null) {
    ...
    const _result_len = c.kreuzberg_rerank_len(_query_z, _documents_z, _config_handle);
    return blk: {
        if (_result == null) return _first_error(KreuzbergError);
        const slice = _result[0.._result_len];
        const owned = try std.heap.c_allocator.dupe(u8, slice);
...
///
/// Since v5.0.
pub fn get_reranker_preset(_name: []const u8) error{OutOfMemory}!?[]u8 {
    const _name_z = try std.fmt.allocPrintSentinel(
        std.heap.c_allocator, "{s}", .{_name}, 0);
    defer std.heap.c_allocator.free(_name_z);
    const _result = c.kreuzberg_get_reranker_preset(_name_z);
    return if (_result == null) null else blk: {
        const _json_ptr = c.kreuzberg_reranker_preset_to_json(_result.?);
        c.kreuzberg_reranker_preset_free(_result.?);
        defer _free_string(_json_ptr);
