...
data class LayoutDetectionConfig(
     ...
     *
     * When a native oxide table and a layout (TATR/SLANeXT) table overlap on the same region, this
     * controls which one is kept. Defaults to `TableOverlapPreference.Content` (historical
     * behavior: keep the table with more content). Set to `TableOverlapPreference.Native` to favor
     * source reading order (higher text F1) over the model's cell reflow.
     */
