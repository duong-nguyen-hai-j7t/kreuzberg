...
object Xberg {
     ...
     *
     * Builds a `crawlberg.CrawlEngine` from `config.crawl`, calls `CrawlEngine.map`, and returns
     * the set of discovered URLs as a `crawlberg.MapResult` (re-exported as `MapResult`).
     *
     * Use this when you need the URL inventory of a site before committing to full document
     * extraction — e.g. to build a crawl queue or validate scope.
     *
     * **Errors:**
     *
     * Returns `Validation` if the crawl configuration fails validation or if the map operation
     * itself fails.
     */
