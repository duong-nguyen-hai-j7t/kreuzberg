// This file is auto-generated by alef — DO NOT EDIT.
// alef:hash:57aa47f0dedb950de05c0a20364f5e79da2e32d02653494dd2f8a611cd9fd74e
// To regenerate: alef generate
// To verify freshness: alef verify --exit-code
// Issues & docs: https://github.com/kreuzberg-dev/alef
#nullable enable

using System;
using System.Collections.Generic;
using System.Text.Json;
using System.Text.Json.Serialization;

namespace Kreuzberg;

/// <summary>
/// OpenDocument metadata from meta.xml
///
/// Contains metadata fields defined by the OASIS OpenDocument Format standard.
/// Uses Dublin Core elements (dc:) and OpenDocument meta elements (meta:).
/// </summary>
public sealed record OdtProperties
{
    /// <summary>
    /// Document title (dc:title)
    /// </summary>
    [JsonPropertyName("title")]
    public string? Title { get; init; } = null;

    /// <summary>
    /// Document subject/topic (dc:subject)
    /// </summary>
    [JsonPropertyName("subject")]
    public string? Subject { get; init; } = null;

    /// <summary>
    /// Current document creator/author (dc:creator)
    /// </summary>
    [JsonPropertyName("creator")]
    public string? Creator { get; init; } = null;

    /// <summary>
    /// Initial creator of the document (meta:initial-creator)
    /// </summary>
    [JsonPropertyName("initial_creator")]
    public string? InitialCreator { get; init; } = null;

    /// <summary>
    /// Keywords or tags (meta:keyword)
    /// </summary>
    [JsonPropertyName("keywords")]
    public string? Keywords { get; init; } = null;

    /// <summary>
    /// Document description (dc:description)
    /// </summary>
    [JsonPropertyName("description")]
    public string? Description { get; init; } = null;

    /// <summary>
    /// Current modification date (dc:date)
    /// </summary>
    [JsonPropertyName("date")]
    public string? Date { get; init; } = null;

    /// <summary>
    /// Initial creation date (meta:creation-date)
    /// </summary>
    [JsonPropertyName("creation_date")]
    public string? CreationDate { get; init; } = null;

    /// <summary>
    /// Document language (dc:language)
    /// </summary>
    [JsonPropertyName("language")]
    public string? Language { get; init; } = null;

    /// <summary>
    /// Generator/application that created the document (meta:generator)
    /// </summary>
    [JsonPropertyName("generator")]
    public string? Generator { get; init; } = null;

    /// <summary>
    /// Editing duration in ISO 8601 format (meta:editing-duration)
    /// </summary>
    [JsonPropertyName("editing_duration")]
    public string? EditingDuration { get; init; } = null;

    /// <summary>
    /// Number of edits/revisions (meta:editing-cycles)
    /// </summary>
    [JsonPropertyName("editing_cycles")]
    public string? EditingCycles { get; init; } = null;

    /// <summary>
    /// Document statistics - page count (meta:page-count)
    /// </summary>
    [JsonPropertyName("page_count")]
    public int? PageCount { get; init; } = null;

    /// <summary>
    /// Document statistics - word count (meta:word-count)
    /// </summary>
    [JsonPropertyName("word_count")]
    public int? WordCount { get; init; } = null;

    /// <summary>
    /// Document statistics - character count (meta:character-count)
    /// </summary>
    [JsonPropertyName("character_count")]
    public int? CharacterCount { get; init; } = null;

    /// <summary>
    /// Document statistics - paragraph count (meta:paragraph-count)
    /// </summary>
    [JsonPropertyName("paragraph_count")]
    public int? ParagraphCount { get; init; } = null;

    /// <summary>
    /// Document statistics - table count (meta:table-count)
    /// </summary>
    [JsonPropertyName("table_count")]
    public int? TableCount { get; init; } = null;

    /// <summary>
    /// Document statistics - image count (meta:image-count)
    /// </summary>
    [JsonPropertyName("image_count")]
    public int? ImageCount { get; init; } = null;


    /// <summary>
    /// Parse a <see cref="OdtProperties"/> from a JSON string.
    /// </summary>
    /// <exception cref="KreuzbergException">When the JSON cannot be deserialised.</exception>
    public static OdtProperties FromJson(string json)
    {
        try
        {
            return JsonSerializer.Deserialize<OdtProperties>(json, JsonOptions)
                ?? throw new KreuzbergException($"Failed to parse OdtProperties from JSON: deserializer returned null");
        }
        catch (KreuzbergException)
        {
            throw;
        }
        catch (Exception e)
        {
            throw new KreuzbergException($"Failed to parse OdtProperties from JSON: {e.Message}", e);
        }
    }

    private static readonly JsonSerializerOptions JsonOptions = new()
    {
        DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingDefault,
        Converters = { new JsonStringEnumConverter(JsonNamingPolicy.SnakeCaseLower) },
    };
}
