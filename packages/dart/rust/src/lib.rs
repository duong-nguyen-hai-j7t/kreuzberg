...
pub fn get_embedding_preset(_name: String) -> Option<EmbeddingPreset> {
    kreuzberg::get_embedding_preset(&_name).map(EmbeddingPreset::from)
}
...
pub fn rerank(
    ...
    kreuzberg::rerank(_query, _documents, &kreuzberg::RerankerConfig::from(_config))
        .map(|v| v.into_iter().map(RerankedDocument::from).collect::<Vec<_>>())
        .map_err(|e| e.to_string())
}
...
pub fn get_reranker_preset(_name: String) -> Option<RerankerPreset> {
    kreuzberg::get_reranker_preset(&_name).map(RerankerPreset::from)
}
