...
- **ocr**: `candle-trocr` feature enabling Microsoft TrOCR (transformer-based printed/handwritten text recognition) via candle. Wires TrOCR encoder-decoder model behind new `candle-trocr` aggregate feature; backend selectable via `--ocr-backend candle-trocr`. Sub-features on `kreuzberg-candle-ocr` for future VLM backends (GOT-OCR 2.0, GLM-OCR, PaddleOCR-VL).
