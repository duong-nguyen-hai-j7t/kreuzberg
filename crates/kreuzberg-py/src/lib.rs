...
#[allow(clippy::missing_errors_doc)]
#[pyfunction]
#[pyo3(signature = (_name, _cache_dir=None))]
pub fn download_model(_name: String, _cache_dir: Option<String>) -> PyResult<String> {
...
#[pyfunction]
#[pyo3(signature = (_name))]
pub fn get_embedding_preset(_name: String) -> Option<EmbeddingPreset> {
    kreuzberg::get_embedding_preset(&_name).map(Into::into)
}
...
pub fn rerank(_query: String, _documents: Vec<String>, _config: RerankerConfig) -> PyResult<Vec<RerankedDocument>> {
    let _config_core: kreuzberg::RerankerConfig = _config.into();

    kreuzberg::rerank(_query, _documents, &_config_core)
        .map(|val| val.into_iter().map(Into::into).collect())
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
pub fn get_reranker_preset(_name: String) -> Option<RerankerPreset> {
    kreuzberg::get_reranker_preset(&_name).map(Into::into)
}
